...
function isDeveloperUser(user: Record<string, unknown> | null): boolean {
  ...
  if (candidates.some((c) => c.includes("developer"))) return true;
  if (typeof window === "undefined") return false;
  try {
    return (localStorage.getItem("userType") ?? "").toLowerCase().includes("developer");
  } catch {
    return false;
  }
}

export default function DashboardInspectionRepresentativesPage() {
  const { user } = useUserContext();
  const router = useRouter();
  const allowed = useMemo(() => isDeveloperUser(user as Record<string, unknown> | null), [user]);
