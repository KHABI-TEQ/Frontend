...
const NewHeroSection = () => {
  ...
  // Get hero video URLs from settings with demo fallbacks
  const heroVideos = [
    homePageSettings?.hero_video_1_url,
    homePageSettings?.hero_video_2_url,
  ].filter(Boolean).length > 0
    ? [homePageSettings?.hero_video_1_url, homePageSettings?.hero_video_2_url].filter(Boolean) as string[]
    : [
  ...
  try {
    return (
    <section className='w-full min-h-screen bg-gradient-to-br from-[#0B423D] via-[#0A3D6D] to-[#0A3E72] flex items-center overflow-hidden relative pt-24 sm:pt-28 lg:pt-32'>
      {/* Background decorative elements */}
      <div className="absolute inset-0 bg-[url('/grid-pattern.svg')] opacity-[0.08]"></div>
      <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent"></div>

      <div className='w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 py-12 sm:py-16 lg:py-20 relative z-10'>
        {/* Hero Content Component */}
        <HeroContent showTrustStrip={false} />

        {/* Hero video slider with autoplay - show skeleton only while loading */}
        {settingsLoading ? (
          <div className='mt-12 sm:mt-16 relative'>
            <div className='bg-white/5 rounded-2xl p-3 sm:p-4 max-w-2xl mx-auto border border-white/10'>
              <div className='aspect-video rounded-xl bg-white/10 animate-pulse'></div>
            </div>
          </div>
        ) : heroVideos.length === 0 ? (
          <div className='mt-12 sm:mt-16 relative'>
            <div className='bg-white/5 rounded-2xl p-3 sm:p-4 max-w-2xl mx-auto border border-white/10'>
              <div className='aspect-video rounded-xl bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center'>
          ...
          <motion.div
            ...
            <div className='bg-white/5 backdrop-blur-md rounded-2xl p-3 sm:p-4 max-w-2xl mx-auto border border-white/10 shadow-2xl'>
